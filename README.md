...
> 1211+ 单测覆盖。详细产品 / 阶段计划 / 架构 → [docs/](docs/) (含 PRD 全集)。
...
- 条件格式化:4 种规则(threshold / dataBar / topN / bottomN);透视/明细双模隔离;cell/row 双 scope — 详见 [docs/conditional-format-design.md](docs/conditional-format-design.md)
- 列宽拖拽;冻结列头 / 行头 sticky
...
### UI 形态
- **撤销 / 重做**(P5+):snapshot 风格,快捷键 `Cmd/Ctrl+Z` / `Cmd/Ctrl+Shift+Z`;数据源切换自动清空 history — 详见 [docs/undo-redo-design.md](docs/undo-redo-design.md)
- **浏览模式**:沉浸视图,隐藏所有 chrome,只看数据 — 右上角浮动退出按钮 / Esc
- 三面板独立可收起(工具栏 / 字段面板 / 字段树),localStorage 持久化
- 字段图标按数据类型显示(`Aa`/`#`/calendar/`✓`)— FieldTree + DropZones chip 共用
- **重复 chip 警告**:同字段拖多次 chip 红边框 + ⚠;buildQuery 翻译时 first-wins dedup,改 chip agg/qc 警告自动清
- 列头右键菜单:升序/降序/取消 / 复制 / (adhoc 数值列)条件格式化
- 行/列头成员右键菜单:筛选=X(In)/ 排除此项(NotIn)/ 复制
- chip 右键菜单(zone 内字段):排序 / 移动 / 快速计算 / 显示合计/小计(互斥) / 条件格式化 / 删除
  - 第 1 个维度 chip 显示"合计",其余显示"小计" — 互斥但 query 等价(都改 per-field subTotal)
- 设置面板:全表总计 / 小计位置 / 冻结 / 压缩空行/列 / 翻页模式 / 显示模式 等
- **单元格查看明细**(透视):右键 cell 钻取明细 — 只带该 cell 对应的 measure(不再 dump 所有度量)
...
│   ├── conditionalFormat/  threshold / dataBar / topN / bottomN 评估 + row-scope 预算
│   ├── metadata/           Metadata 索引 + 数据类型派生(给 FieldTree/DropZones icon 用)
...
```
docs/
├── prd/                              产品 PRD + 阶段计划(归拢自外部 prd/,2026-05-15)
│   ├── README.md                     PRD 总览 / 索引
│   ├── 1-product.md                  产品定义
│   ├── 2-architecture.md             架构设计(代码注释指向)
│   ├── 3-nfr-backend.md              非功能需求 + 后端契约
│   ├── phase-p0.md ~ phase-p3-plus.md 各阶段 PRD
│   ├── appendices.md
│   └── engineering/                  开发计划(p0-dev / p1-dev / p2-dev / p3-plus-dev)
├── adr-004-hierarchy-drill.md        Hierarchy drill C2 策略 finding(原 ADR-004-finding.md)
├── conditional-format-design.md      条件格式化:4 种规则 × 双模 × 双 scope
├── ui-features-p5.md                 P5+ UI 小变更合集
└── undo-redo-design.md               撤销 / 重做 snapshot 风格
```

## ADR / 关键设计决定

- [ADR-004 — Hierarchy drill](docs/adr-004-hierarchy-drill.md) — 用 OLAP 轴深度而非 filter
- [条件格式化设计](docs/conditional-format-design.md) — 4 种规则 × 双模隔离 × 双 scope
- [P5+ UI 变更合集](docs/ui-features-p5.md) — 字段类型 icon / 合计-小计互斥 / 设置面板归属 / 重复 chip 警告 / filter group 展平 / drill-through 单 measure
- [撤销 / 重做设计](docs/undo-redo-design.md) — snapshot 风格;翻页不算 step;metadata 切换自动清 history
